...
# -----------------------------
def fuzzy_trust(sats, hdop, jump):
    ...
    # Fuzzy rules
    rules = []
    rules.append((max(sat_few, hd_poor),0))
    rules.append((j_imp,0))
    rules.append((min(sat_ok, hd_good, j_norm),1))
    rules.append((min(sat_many, hd_ex, j_norm),2))
    ...
    # Ensure non-zero minimum for smooth output
    if out_agg.sum()==0: return 0.05
...
def main():
    ...
    try:
        while True:
            ...
            # Fuzzy fallback
            fuzzy_score = fuzzy_trust(sats, hdop if hdop is not None else 99.0, min(jump_m,100.0))
